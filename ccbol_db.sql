...
--
-- Servidor: localhost
-- Tiempo de generación: 06-09-2017 a las 23:33:36
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `listEvent` ()  BEGIN
	IF(SELECT EXISTS(SELECT * FROM event))THEN
		SELECT ev.id, ev.title, ev.description, ev.date, ev.date, ev.start_time, ev.finish_time, ex.id as id_expositor, ex.name, ex.last_name, lo.site, lo.venue FROM event ev INNER JOIN expositor ex ON ev.id_expositor=ex.id INNER JOIN location lo ON ev.id_location=lo.id;
    ELSE
		SELECT 'yes' AS error, 'No existen Actividades' AS respuesta; 
    END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listEvents` ()  BEGIN
	IF(SELECT EXISTS(SELECT * FROM event))THEN
		SELECT ev.id, ev.title, ev.description, ev.date, ev.date, ev.start_time, ev.finish_time, ex.id, ex.name, ex.last_name, lo.site, lo.venue FROM event ev INNER JOIN expositor ex ON ev.id_expositor=ex.id INNER JOIN location lo ON ev.id_location=lo.id;
    ELSE
		SELECT 'yes' AS error, 'No existen Actividades' AS respuesta; 
    END IF;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `listExpositors` ()  BEGIN
	IF(SELECT EXISTS(SELECT * FROM expositor))THEN
		SELECT id, name, last_name, photo, company, description, facebook, twitter, linkedin, github, other  FROM expositor;
    ELSE
		SELECT 'yes' as error, 'No existen expositores' as respuesta;
    END IF;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `listLodging` ()  BEGIN
    ...
    ELSE
		SELECT 'yes' AS error, 'No existen alojamientos' AS respuesta; 
    END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listLodgings` ()  BEGIN
	IF(SELECT EXISTS(SELECT * FROM lodging))THEN
		SELECT lt.latitude, lt.longitude, lt.site, lt.venue, 
    ...
    ELSE
		SELECT 'yes' AS error, 'No existen Hospedajes' AS respuesta ; 
    END IF;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `listSponsor` ()  BEGIN
	IF(SELECT EXISTS(SELECT * FROM sponsor))THEN
		SELECT * from sponsor;
    ELSE
		SELECT 'yes' AS error, 'No existen Sponsors' AS respuesta; 
    END IF;
END$$
...
(16, '2017-08-21 04:37:51', '2017-08-31 04:43:19', 1),
(17, '2017-08-31 04:27:16', NULL, 1);
...
(5, 'Inscripción. Registro y Acreditación', 'Participantes Ccbol 2017', '2017-09-11', '08:00:00', '17:00:00', 13, 18),
...
(7, 'Acto de Inauguración', 'Autoridades, Conferencistas, Invitados Especiales Delegaciones Universitarias Participantes CCBol2017', '2017-10-11', '18:30:00', '20:00:00', 13, 20),
(8, 'Cocktail de Bienvenida', 'Autoridades, Conferencistas, Invitados Especiales', '2017-09-11', '21:00:00', '22:00:00', 13, 19),
(9, 'Fiesta de Bienvenida', 'Delegaciones Universitarias Participantes CCBol2017', '2017-09-11', '22:00:00', '02:00:00', 13, 21),
...
(13, 'Mesa Redonda 1: “El nuevo paradigma de la educación superior basada en la Internet”', '', '2017-09-12', '09:30:00', '12:30:00', 13, 19),
...
(19, 'Mesa de Trabajo Sociedad Científica de Estudiantes de Sistemas e Informática', '', '2017-09-12', '15:00:00', '18:30:00', 13, 19),
(20, 'Laboratorio Fundación Jala: “Behavior Driven Depelopment, a Hands-On in Java”', 'Raul Garvizu', '2017-09-12', '16:30:00', '18:30:00', 13, 18),
...
(22, 'Competencia de Robótica', 'Convocatoria especifica', '2017-09-13', '09:00:00', '12:30:00', 13, 18),
...
(24, 'Mesa Redonda 2: “Lenguajes de programación apropiados para la enseñanza introductoria de la programación”', '', '2017-09-13', '09:00:00', '12:30:00', 13, 19),
...
(26, 'Feria de Innovación Tecnológica Sociedad Científica de Estudiantes de Sistemas e Informática', '', '2017-09-13', '15:00:00', '18:30:00', 13, 19),
...
(30, 'Cena de Gala: Autoridades, Conferencistas, Invitados Especiales', '', '2017-09-13', '20:00:00', '22:00:00', 13, 21),
(31, 'Peña Folklórica. Autoridades, Conferencistas, Invitados Especiales', 'Delegaciones Universitarias Participantes CCBol2017', '2017-09-13', '20:00:00', '02:00:00', 13, 21),
(59, 'Presentación Trabajos', 'Convocatoria especifica', '2017-09-14', '09:00:00', '12:30:00', 13, 18),
...
(61, 'Mesa Redonda 3: “Los perfiles del Ingeniero Informático, Ingeniero en Redes e Ingeniero en Ciencias de la Computación”', '', '2017-09-14', '09:00:00', '12:30:00', 13, 19),
...
(63, 'Feria de Innovación Tecnológica Sociedad Científica de Estudiantes de Sistemas e Informática', '', '2017-09-14', '15:00:00', '18:30:00', 13, 19),
...
(67, 'Presentación Trabajos', 'Convocatoria especifica', '2017-09-15', '09:00:00', '10:30:00', 13, 18),
...
(69, 'Conclusiones y Plenarias', '', '2017-09-15', '09:00:00', '10:30:00', 13, 19),
(70, 'SESIÓN DE CLAUSURA', 'Entrega de certificados, traspaso a la nueva sede CCBOL2018', '2017-09-15', '11:00:00', '12:30:00', 13, 20),
(71, 'Fiesta de Clausura', 'Delegaciones Universitarias Participantes CCBol2017.', '2017-09-15', '22:00:00', '02:00:00', 13, 21);
...
INSERT INTO `expositor` (`id`, `name`, `last_name`, `photo`, `company`, `description`, `facebook`, `twitter`, `linkedin`, `github`, `other`) VALUES
(2, 'José Daniel', 'Alberto Constan', 'http://ccbol2017.usfx.bo/assets/img/conferencistas/CONSTAN.png', '', 'Capacitador de Instructores Cisco Networking Academy – CCNA, CCAI, ITQ, CCSI Fundación Proydesa. Buenos Aires Argentina', '', '', 'http://ar.linkedin.com/in/daniel-constán-96349023', '', 'http://www.proydesa.org'),
(3, 'Patricio Raúl', 'Carranza', 'http://ccbol2017.usfx.bo/assets/img/conferencistas/CARRANZA.png', '', 'Analista de IoT (Internet of Things).Consultor en eLearning, mLearning, MOOC Miembro de Internet Society y del steering group de IoT de la Cámara Argentina de Internet.', '', '', 'http://ar.linkedin.com/in/pcarranza', '', 'http://www.carranza.com.ar'),
(4, 'Salvador', 'Crespo', 'http://ccbol2017.usfx.bo/assets/img/conferencistas/CRESPO.png', '', 'Director Ejecutivo de la Cámara Argentina de Internet Comisión IoT CABASE. Buenos Aires Argentina', '', '', 'http://www.linkedin.com/in/salvador-crespo', '', 'http://www.cabase.org.ar/comision-internet-de-las-cosas-iot-2/'),
(5, 'Juan Gabriel', 'Gomila Salas', 'http://ccbol2017.usfx.bo/assets/img/conferencistas/GOMILA.png', '', 'CEO en @frogames_sl. Data Scientist, Game Designer & Game Producer, Creador de apps y videojuegos en Frogames. Instructor en un curso online en la creación de apps móviles.', '', '', 'http://www.linkedin.com/in/juan-gabriel-gomila-salas', '', 'http://juangabrielgomila.com/biography/mi-curriculum/'),
(6, 'Carlos Guillermo', 'Bran', 'http://ccbol2017.usfx.bo/assets/img/conferencistas/BRAM.png', 'http://www.udb.edu.sv', 'Master en Gerencia de Tecnología y en Investigación en TI. Director Instituto de investigación e innovación en electrónica. Investigador en sistemas embebidos, IoT y controladores inteligentes. Profesor Universidad Don Bosco en El Salvador', '', '', '', '', 'http://citius.usc.es/equipo/investigadores-en-formacion/carlos-guillermo-bran'),
(7, 'Jorge Miguel', 'Cuellar Rendón', 'http://ccbol2017.usfx.bo/assets/img/conferencistas/CUELLAR.png', 'http://www.behance.net/Jorgemcuellar', 'Character Designer / Art Director / Ilustrator / Game Artist.\r\nDirector de Arte para Dibujos animados y Videojuegos en Buenos Aires, Argentina', '', '', 'http://www.linkedin.com/in/jorge-cuellar-rendon', '', 'http://jorgemcuellar.blogspot.com'),
(8, 'Cesar Jesús ', 'Chávez Martínez', 'http://ccbol2017.usfx.bo/assets/img/conferencistas/CHAVEZ.png', '', 'Consultor en Seguridad Informática. Analista Forense. Gestor del Proyecto Peruhacking y del Bsides Security Conference Perú. Coordinador Red Latinoamericana de Informática Forense', 'http://www.facebook.com/peruhacking', '', 'http://pe.linkedin.com/in/peruhacking', '', 'http://computo-forense.blogspot.com'),
(9, 'Carlos Alberto', 'Olivera Terrazas', 'http://ccbol2017.usfx.bo/assets/img/conferencistas/OLIVERA.png', '', 'Emprendedor IT, Desarrollador de Ecommerce con plataformas Real Time Web en Bolivia, Universidad Católica Boliviana, Bolivia', '', '', 'http://www.linkedin.com/in/colivera/', '', ''),
(10, 'Said Eduardo', 'Pérez Poppe', 'http://ccbol2017.usfx.bo/assets/img/conferencistas/PEREZ.png', 'http://www.sis.usfx.edu.bo', 'Ingeniero de Sistemas y Telecomunicaciones, Docente USFX. Instructor Cisco Networking Academy – CCNA. Presidente comité científico Olimpiadas Bolivianas Robótica. Mentor de equipo boliviano de robótica First Global Challenge. 2017', '', '', '', '', 'http://www.saidperez.com'),
(11, 'Oswaldo Gerardo', 'Velázquez Aroni', 'http://ccbol2017.usfx.bo/assets/img/conferencistas/VELASQUEZ.png', 'http://www.sis.usfx.edu.bo', 'Master en Investigación en IT. Instructor Cisco Networking Academy – CCNA, ITEssentials. Docente y Encargado Departamento de Inteligencia Artificial USFX.', '', '', '', '', ''),
(12, 'Expositores de Fundación Jala', '', 'http://ccbol2017.usfx.bo/assets/img/fundacion-jala.png', 'http://www.fundacion-jala.org/', 'Cochabamba - Bolivia', 'http://www.facebook.com/paginafundacionjala/', '', '', '', ''),
...
INSERT INTO `location` (`id`, `latitude`, `longitude`, `site`, `venue`) VALUES
...
INSERT INTO `sponsor` (`id`, `name`, `image`, `relevance`) VALUES
(1, 'Fancesa', 'http://ccbol2017.usfx.bo/assets/img/sponsors/fancesa-min.jpg', 1),
(2, 'Hotel Real Audiencia', 'http://ccbol2017.usfx.bo/assets/img/sponsors/hotel.jpg', 2),
(3, 'Tarco Tour', 'http://ccbol2017.usfx.bo/assets/img/sponsors/tarco-min.png', 3),
(4, 'Sureña', 'http://ccbol2017.usfx.bo/assets/img/sponsors/sure%C3%B1a-min.png', 4),
(5, 'TV U', 'http://ccbol2017.usfx.bo/assets/img/sponsors/tvu-min.png', 5),
(6, 'JoyRide', 'http://ccbol2017.usfx.bo/assets/img/sponsors/joy-min.jpg', 6);
